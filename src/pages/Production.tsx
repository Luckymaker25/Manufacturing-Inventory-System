...
import { Factory, AlertCircle, ClipboardList, TrendingUp, AlertTriangle, CheckCircle2, Printer, PlusCircle, Play, CheckCircle, XCircle, Loader2, ShoppingCart, LayoutDashboard, Clock } from 'lucide-react';
import { cn } from '@/lib/utils';
import { motion, AnimatePresence } from 'motion/react';

// --- Section A: PPIC / Planning ---
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-sm font-bold text-slate-700 uppercase tracking-wider">Finished Good SKU</label>
            <select
          ...
          <div className="space-y-2">
            <label className="text-sm font-bold text-slate-700 uppercase tracking-wider">Target Quantity</label>
            <input
              type="number"
              min="1"
              value={targetQty}
...
const PendingSalesQueue = ({ onPreview, onRefresh }: { onPreview: (item: PendingSalesProduction) => void, onRefresh: () => void }) => {
  ...
  const [confirmId, setConfirmId] = useState<number | null>(null); // State baru untuk konfirmasi
  ...
  const handleRequestRelease = async (item: PendingSalesProduction) => {
    // Perbaikan ID: Antisipasi nama variabel dari backend (id atau order_id)
    const currentId = (item as any).id || item.order_id;
    ...
    // Logika Konfirmasi Cerdas (Tanpa window.confirm bawaan browser)
    ...
    // Jika tombol diklik untuk kedua kalinya:
    setFulfillingId(currentId);
    setConfirmId(null); // Reset confirm state
    
    try {
      await api.requestFulfillment(currentId);
      loadData();
      onRefresh();
    } catch (err: any) {
          ...
          <tbody className="divide-y divide-slate-50">
            {loading ? (
              ...
              pending.map((item) => {
                ...
                return (
                  <tr key={`${currentId}-${item.product_id}`} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-8 py-4 text-slate-500">{new Date(item.date).toLocaleDateString()}</td>
                    <td className="px-8 py-4 font-mono text-xs font-bold text-slate-900">{item.ref_no}</td>
                    <td className="px-8 py-4 text-slate-600 font-medium">{item.customer_name || 'Walk-in Customer'}</td>
                    ...
                    <td className="px-8 py-4 text-right font-bold text-slate-900">{item.target_qty}</td>
                    <td className="px-8 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        
                        {/* TOMBOL REQUEST RELEASE YANG TELAH DIPERBAIKI */}
                        <button
                          type="button"
                          onClick={(e) => {
                            e.stopPropagation();
                            handleRequestRelease(item);
                          }}
                          disabled={fulfillingId === currentId}
                          className={cn(
                            "relative z-10 inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-[10px] font-bold transition-all shadow-sm cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed",
                            confirmId === currentId 
                              ? "bg-rose-500 hover:bg-rose-600 text-white animate-pulse" // Merah saat butuh konfirmasi
                              : hasStock 
                                ? "bg-emerald-600 hover:bg-emerald-700 text-white" 
                                : "bg-amber-500 hover:bg-amber-600 text-white"
                          )}
                        >
                          {fulfillingId === currentId ? (
                          ...
                          {/* Ubah teks jika sedang tahap konfirmasi */}
                          {confirmId === currentId ? "Are you sure?" : "Request Release"}
                        </button>
        ...
        <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-slate-500">Review BOM for {product.name} ({targetQty} {product.unit})</p>
            </div>
          </div>
                ...
                {bom.map((item) => (
                  <div key={item.id} className="flex items-center justify-between p-4 rounded-2xl border border-slate-100 bg-slate-50/30">
                    <div>
                      ...
                      <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Unit Requirement: {item.quantity} {item.unit}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-lg font-black text-indigo-600">{(item.quantity * targetQty).toFixed(2)}</div>
                      <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Total Required ({item.unit})</div>
                    </div>
                  </div>
                ))}
                {bom.length === 0 && (
...
// --- Section C: Production Board (Execution Report) ---
const CompleteWOModal = ({ wo, onClose, onCompleted }: { wo: WorkOrder, onClose: () => void, onCompleted: () => void }) => {
  ...
  const handleComplete = async () => {
    ...
    setError('');
    try {
      const actual_materials = Object.entries(actualQtys).map(([id, qty]) => ({
        ...
        <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-slate-500">Batch: <span className="text-indigo-600 font-bold">{wo.batch_id}</span> • {wo.product_name}</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-slate-200 rounded-full transition-colors text-slate-400">
            <XCircle className="h-8 w-8" />
        ...
        <div className="p-8 space-y-10 overflow-y-auto flex-1">
          ...
          <section>
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-1 rounded-lg">BOM SNAPSHOT</span>
            </div>
            <div className="grid grid-cols-1 gap-3">
              {materials.map((mat) => (
                <div key={mat.id} className="group grid grid-cols-1 md:grid-cols-12 gap-4 items-center p-4 rounded-2xl border border-slate-100 bg-slate-50/30 hover:bg-white hover:shadow-xl hover:shadow-slate-100 transition-all">
                  <div className="md:col-span-5">
                    <div className="font-bold text-slate-900 text-sm">{mat.material_name}</div>
                    <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Planned: {mat.planned_qty} {mat.unit}</div>
                  </div>
                  <div className="md:col-span-7 flex items-center gap-4">
                    <div className="relative flex-1">
                      <input
                        type="number"
                        value={actualQtys[mat.raw_material_id] || 0}
                        onChange={(e) => setActualQtys(prev => ({ ...prev, [mat.raw_material_id]: Number(e.target.value) }))}
                      ...
                      />
                      <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-300 uppercase">{mat.unit}</span>
                    </div>
                    <div className={cn(
                      "text-[10px] font-black px-2 py-1 rounded-lg",
                      (actualQtys[mat.raw_material_id] || 0) > mat.planned_qty ? "bg-amber-50 text-amber-600" : "bg-emerald-50 text-emerald-600"
                    )}>
                      {((actualQtys[mat.raw_material_id] || 0) - mat.planned_qty).toFixed(1)} VAR
                    </div>
                  </div>
                </div>
              ))}
          ...
          <section className="bg-slate-900 rounded-[2rem] p-8 text-white">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="text-[10px] font-bold text-slate-400">TARGET: {wo.target_qty} {wo.unit}</div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-3">
                <label className="block text-[10px] font-black text-emerald-400 uppercase tracking-widest">Good Quantity (Passed QC)</label>
                <div className="relative">
                  ...
                  />
                  <span className="absolute right-6 top-1/2 -translate-y-1/2 text-xs font-black text-emerald-500/40 uppercase">{wo.unit}</span>
                </div>
              </div>
              <div className="space-y-3">
                <label className="block text-[10px] font-black text-red-400 uppercase tracking-widest">Reject / Scrap Quantity</label>
                <div className="relative">
                  <input
                    type="number"
                    value={rejectQty}
                  ...
                  />
                  <span className="absolute right-6 top-1/2 -translate-y-1/2 text-xs font-black text-red-500/40 uppercase">{wo.unit}</span>
                </div>
              </div>
            </div>
          </section>
...
const ProductionBoard = ({ orders, onUpdate }: { orders: WorkOrder[], onUpdate: () => void }) => {
    ...
    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-slate-50">
            {orders.filter(o => ['Approved', 'WIP'].includes(o.status)).length === 0 ? (
              <tr>
                <td colSpan={7} className="px-8 py-12 text-center text-slate-400">
              ...
              orders.filter(o => ['Approved', 'WIP'].includes(o.status)).map((order) => (
                <tr key={order.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-8 py-4 font-black text-slate-900">{order.batch_id}</td>
                  <td className="px-8 py-4">
                    <span className={cn(
                      "px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest",
                      order.source_type === 'Sales' ? "bg-purple-50 text-purple-600 border border-purple-100" : "bg-blue-50 text-blue-600 border border-blue-100"
                    )}>
                      {order.source_type}
                    </span>
                  </td>
                  <td className="px-8 py-4 font-mono text-xs font-bold text-slate-400 group-hover:text-slate-900 transition-colors">{order.reference_no}</td>
                  <td className="px-8 py-4">
                    <div className="font-bold text-slate-900">{order.product_name}</div>
                    <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{order.sku}</div>
                  </td>
                  ...
                  <td className="px-8 py-4 text-center">
                    <span className={cn(
                      ...
                      order.status === 'Pending' && "bg-slate-100 text-slate-500 border border-slate-200",
                      order.status === 'Approved' && "bg-amber-50 text-amber-600 border border-amber-100",
                      order.status === 'WIP' && "bg-blue-50 text-blue-600 border border-blue-100"
                    )}>
                      {order.status === 'Pending' && <Clock className="h-3 w-3" />}
                      {order.status === 'Approved' && <CheckCircle className="h-3 w-3" />}
                      {order.status === 'WIP' && <Play className="h-3 w-3 animate-pulse" />}
                      {order.status === 'Pending' ? 'Waiting Approval' : order.status}
                    </span>
                  </td>
                  <td className="px-8 py-4 text-right">
                    ...
                    {order.status === 'Pending' && (
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">Waiting Warehouse</span>
                    )}
                  </td>
                </tr>
              ))
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-slate-500 font-bold uppercase text-xs tracking-[0.3em] mt-1">Production Control Center</p>
          </div>
        </div>
        ...
        <div className="flex p-1.5 bg-slate-100 rounded-2xl border border-slate-200">
          <button
            ...
            className={cn(
              ...
              activeTab === 'planning' ? "bg-white text-slate-900 shadow-sm" : "text-slate-400 hover:text-slate-600"
            )}
          >
            PPIC / Planning
          </button>
          <button
            ...
            className={cn(
              "px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all",
              activeTab === 'execution' ? "bg-white text-slate-900 shadow-sm" : "text-slate-400 hover:text-slate-600"
            )}
          >
            Production Board
          </button>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {activeTab === 'planning' && (
            <>
              <MakeToStockForm onPreview={(product, qty) => setPreviewData({ product, qty, sourceType: 'Restock', referenceNo: 'AUTO' })} />
              <PendingSalesQueue 
                onRefresh={loadWorkOrders}
                onPreview={(item) => setPreviewData({ 
                  product: { id: item.product_id, name: item.product_name, unit: 'pcs' }, // unit fallback
                  qty: item.target_qty, 
                  sourceType: 'Sales', 
          ...
          {activeTab === 'execution' && <ProductionBoard orders={workOrders} onUpdate={loadWorkOrders} />}
        </motion.div>
      </AnimatePresence>
